...
import { Trash2, Edit, LogOut, Check, X as XIcon, Star } from 'lucide-react';
    ...
    <div className="w-full max-w-7xl mx-auto px-6 py-12 animate-fade-in bg-white min-h-screen text-black">
      ...
      <div className="flex gap-8 mb-8 border-b border-gray-100">
        {['inventory', 'orders'].map(tab => (
           ...
           >
             {tab === 'inventory' ? 'Inventario' : 'Pedidos'}
           </button>
        ))}
      </div>
        ...
        <div>
          <div className="flex justify-between mb-8">
             <input 
               ...
               className="bg-white border border-gray-200 p-3 rounded w-full max-w-md focus:border-black outline-none text-black placeholder-gray-400"
             />
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input value={editingProduct.name} onChange={e => setEditingProduct({...editingProduct, name: e.target.value})} className="w-full border border-gray-200 p-2 focus:border-navy-900 outline-none" required />
                    </div>
                    <div>
                        <label className="block text-xs font-bold uppercase text-gray-500 mb-1">Marca</label>
                        <input value={editingProduct.brand} onChange={e => setEditingProduct({...editingProduct, brand: e.target.value})} className="w-full border border-gray-200 p-2 focus:border-navy-900 outline-none" required />
                    </div>
               </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                      ...
                      <input type="number" value={editingProduct.price} onChange={e => setEditingProduct({...editingProduct, price: Number(e.target.value)})} className="w-full border border-gray-200 p-2 focus:border-navy-900 outline-none" required />
                  </div>
                  <div>
                      <label className="block text-xs font-bold uppercase text-gray-500 mb-1">Precio Oferta (Opcional)</label>
                      <input type="number" value={editingProduct.offerPrice || ''} onChange={e => setEditingProduct({...editingProduct, offerPrice: Number(e.target.value)})} className="w-full border border-gray-200 p-2 focus:border-navy-900 outline-none" />
                  </div>
               </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   <div>
                       ...
                       <select value={editingProduct.category} onChange={e => setEditingProduct({...editingProduct, category: e.target.value as any})} className="w-full border border-gray-200 p-2 focus:border-navy-900 outline-none">
               ...
               {editingProduct.category === ProductCategory.PERFUME && (
                 <div className="bg-slate-50 p-6 rounded border border-slate-100 space-y-4">
                   ...
                   <div>
                       ...
                       <select value={editingProduct.subCategory} onChange={e => setEditingProduct({...editingProduct, subCategory: e.target.value as any})} className="w-full border border-gray-200 p-2 bg-white">
                   ...
                   {editingProduct.isDecantAvailable && (
                     <div className="grid grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" value={editingProduct.decantPrice3ml || ''} onChange={e => setEditingProduct({...editingProduct, decantPrice3ml: Number(e.target.value)})} className="w-full border border-gray-200 p-2 text-sm" />
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold uppercase text-gray-500 mb-1">Precio 5ml</label>
                            <input type="number" value={editingProduct.decantPrice5ml || ''} onChange={e => setEditingProduct({...editingProduct, decantPrice5ml: Number(e.target.value)})} className="w-full border border-gray-200 p-2 text-sm" />
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold uppercase text-gray-500 mb-1">Precio 10ml</label>
                            <input type="number" value={editingProduct.decantPrice10ml || ''} onChange={e => setEditingProduct({...editingProduct, decantPrice10ml: Number(e.target.value)})} className="w-full border border-gray-200 p-2 text-sm" />
                        </div>
                     </div>
                   )}
                 </div>
               ...
               <div className="space-y-4">
                    ...
                    <div>
                        <label className="block text-xs font-bold uppercase text-gray-500 mb-1">URL Imagen Principal (Portada)</label>
                        <input value={editingProduct.image} onChange={e => setEditingProduct({...editingProduct, image: e.target.value})} className="w-full border border-gray-200 p-2 focus:border-navy-900 outline-none" />
                    ...
                    <div>
                        <label className="block text-xs font-bold uppercase text-gray-500 mb-1">Galería de Imágenes (Una URL por línea)</label>
                        <textarea 
                            value={editingProduct.gallery?.join('\n') || ''} 
                            onChange={handleGalleryChange}
                            className="w-full border border-gray-200 p-2 h-32 focus:border-navy-900 outline-none text-sm font-mono"
               ...
               <div>
                   ...
                   <textarea value={editingProduct.description} onChange={e => setEditingProduct({...editingProduct, description: e.target.value})} className="w-full border border-gray-200 p-2 focus:border-navy-900 outline-none" rows={4}></textarea>
               </div>
               ...
               <div className="flex justify-end gap-2 mt-8 pt-4 border-t border-gray-100">
                  ...
                  <button type="submit" className="px-8 py-3 bg-navy-900 text-white font-bold hover:bg-navy-800 uppercase tracking-widest text-xs">Guardar Cambios</button>
               </div>
            </form>
          </div>
        </div>
