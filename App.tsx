...
import { Instagram, Mail, Phone, Truck, Award, CreditCard, Lock } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // Initialize Data
  useEffect(() => {
    const savedProducts = localStorage.getItem('soberano_products');
  ...
  // Persistence
  useEffect(() => { if (products.length > 0) localStorage.setItem('soberano_products', JSON.stringify(products)); }, [products]);
  ...
  const handleUpdateHeroImages = (type: 'WATCH' | 'PERFUME', images: string[]) => {
    ...
    showToast('Galería actualizada', 'info');
  };
          ...
          <div className="flex flex-col md:flex-row h-[calc(100vh-96px)]">
            ...
            >
              ...
              <div className="absolute inset-0 flex flex-col justify-center items-center text-center p-8 bg-black/20 group-hover:bg-black/30 transition-colors">
                <h2 className="text-4xl md:text-6xl font-header text-white mb-8 tracking-[0.1em] font-bold drop-shadow-xl">RELOJES</h2>
                {/* Removed Free Shipping Text as requested */}
                <button className="text-white text-sm md:text-base tracking-[0.2em] font-bold border border-white/60 px-10 py-4 hover:bg-navy-900 hover:border-navy-900 hover:text-white transition-all duration-500 uppercase">
                  Ver Colección
                </button>
              </div>
            </div>
            ...
            >
              ...
              <div className="absolute inset-0 flex flex-col justify-center items-center text-center p-8 bg-black/20 group-hover:bg-black/30 transition-colors">
                <h2 className="text-4xl md:text-6xl font-header text-white mb-8 tracking-[0.1em] font-bold drop-shadow-xl">PERFUMES</h2>
                <button className="text-white text-sm md:text-base tracking-[0.2em] font-bold border border-white/60 px-10 py-4 hover:bg-navy-900 hover:border-navy-900 hover:text-white transition-all duration-500 uppercase">
                  Descubrir Aromas
                </button>
              </div>
        ...
        <div className="min-h-screen bg-slate-50 text-slate-800 font-sans selection:bg-navy-900 selection:text-white animate-fade-in flex flex-col">
          ...
          {view !== 'ADMIN' && (
            <footer className="bg-slate-100 border-t border-slate-200 mt-auto">
              ...
              {/* Top Section: Benefits - Navy/Grey theme */}
              <div className="max-w-7xl mx-auto py-16 px-6 grid grid-cols-1 md:grid-cols-3 gap-8 text-center border-b border-slate-200">
                <div className="flex flex-col items-center group">
                  <div className="w-16 h-16 rounded-full bg-white flex items-center justify-center text-navy-900 mb-4 shadow-sm group-hover:bg-navy-900 group-hover:text-white transition-colors duration-500">
                    <Truck size={28} strokeWidth={1.5} />
                  </div>
                  <h4 className="font-header font-bold text-navy-900 uppercase tracking-widest mb-2">ENVÍOS RÁPIDOS</h4>
                  <p className="text-slate-500 text-sm font-light">¡Comprás hoy, lo disfrutás hoy!</p>
                </div>
                ...
                <div className="flex flex-col items-center group">
                  ...
                  <h4 className="font-header font-bold text-navy-900 uppercase tracking-widest mb-2">PRODUCTOS ORIGINALES</h4>
                  <p className="text-slate-500 text-sm font-light">Perfumes originales, de distribuidores legales y certificados.</p>
                </div>
                ...
                <div className="flex flex-col items-center group">
                  ...
                  <h4 className="font-header font-bold text-navy-900 uppercase tracking-widest mb-2">FACILIDADES DE PAGO</h4>
                  <p className="text-slate-500 text-sm font-light">En efectivo, transferencia bancaria, tarjeta de crédito o débito.</p>
                </div>
              </div>

              {/* Bottom Section: Links & Contact */}
              <div className="max-w-7xl mx-auto py-12 px-6 grid grid-cols-1 md:grid-cols-3 gap-12 text-slate-600">
                
                {/* Contact */}
                <div className="text-center md:text-left">
                   <h5 className="text-navy-900 font-bold text-xs tracking-[0.2em] uppercase mb-6">Contacto</h5>
                   <div className="space-y-3 text-sm">
                     ...
                     <p className="flex items-center justify-center md:justify-start gap-3"><Instagram size={16} className="text-navy-900"/> @soberano.py</p>
                   </div>
                </div>

                {/* Links */}
                <div className="text-center">
                   <h5 className="text-navy-900 font-bold text-xs tracking-[0.2em] uppercase mb-6">Enlaces Rápidos</h5>
                   <ul className="space-y-3 text-sm">
                      <li className="hover:text-navy-900 cursor-pointer transition">Políticas de Envío</li>
                      <li className="hover:text-navy-900 cursor-pointer transition">Devoluciones</li>
                      <li className="hover:text-navy-900 cursor-pointer transition">Privacidad</li>
                   </ul>
                </div>
                ...
                <div className="text-center md:text-right flex flex-col justify-between">
                   <div>
                     <h3 className="font-header text-2xl font-bold text-navy-900 tracking-widest mb-2">SOBERANO</h3>
              ...
              {/* Hidden Admin Button - Only on HOME view footer */}
              {view === 'HOME' && (
                <div className="bg-slate-200 py-2 text-center">
                    <button 
                      onClick={() => setView('ADMIN')}
                      className="text-slate-400 hover:text-navy-900 text-[10px] transition-colors"
                      title="Admin"
                    >
                      <Lock size={10} />
                    </button>
          ...
          {/* Floating WhatsApp Button - Navy Blue */}
          <button 
            onClick={openWhatsApp}
            className="fixed bottom-6 right-6 z-40 bg-navy-900 hover:bg-navy-800 text-white p-3 rounded-full shadow-lg shadow-navy-900/30 border border-white/20 transition-transform duration-300 hover:scale-110 flex items-center justify-center"
